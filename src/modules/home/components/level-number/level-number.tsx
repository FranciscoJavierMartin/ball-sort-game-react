import { getValueFromCache } from '@/modules/common/helpers/storage';
import { STORAGE_KEYS } from '@/modules/game/constants/storage';
import './level-number.css';
...
export default function LevelNumber() {
  const level = getValueFromCache(STORAGE_KEYS.LEVEL, 1);

  return (
    <div className='level-number'>
      <div className='level-number-crown'>
        <Crown />
      </div>
      <div className='level-number-value'>Level {level}</div>
    </div>
  );
}
